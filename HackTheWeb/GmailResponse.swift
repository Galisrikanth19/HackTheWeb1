...
struct GmailResponse {
    ...
    init(_ innerHTML: Any?) throws {
        ...
        //let authors = try doc.getElementsByClass("cht2").array()
        let titles = try doc.getElementsByClass("chcontenttextci").array()
        
        var emails = [Email]()
        for i in 0..<titles.count {
            //let author = try authors[i].text()
            let title = try titles[i].text()
            let email = Email(author: "Srikanth", title: title)
            emails.append(email)
        }
        self.emails = emails
    }
